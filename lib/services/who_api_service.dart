...
final countryFilter = countries.map((c) => "SpatialDim eq '$c'").join(' or ');
 ...
 int maxYear = allStats.fold(0, (currentMax, stat) => 
        ...
        if (maxYear > 0) {
            ...
            filteredStats = allStats.where((stat) => stat.year == maxYear).toList();
            print('✅ Filtrage appliqué : ${filteredStats.length} données de l\'année $maxYear conservées.');
        } else {
            // Si aucune année n'est trouvée, utiliser toutes les données converties
            filteredStats = allStats;
            print('⚠️ Aucune année trouvée pour le filtrage, ${filteredStats.length} données conservées.');
        }
...
CountryStats(
 ...
 throw Exception( 'Erreur récupération indicateurs: ${response.statusCode}',
 );
 }
 } catch (e) {
...
Future<void> syncAndSaveCancerStats() async {
 ...
 print('✅ WHOApiService: ${retrievedStats.length} statistiques sauvegardées localement.');
 } else {
